...
class GreedyAgent:
    def __init__(self, size, player_id, random_state=42, cache_data=None):
    ...
    def select_move(self, env: ArrayGomoku):
        episode_len = len(env.states)
        if episode_len == 1:
            m, n = ensure_tuple(env.last_move, self.size)
            row_range = [m - 1, m, m + 1]
            col_range = [n - 1, n, n + 1]
        else:
        ...
        eval_list = []
        where_black = np.where(board == 1)
        where_white = np.where(board == 2)
        ...
        for row, col in feasible_positions:
            ...
            if position_idx not in env.states:
                # 假设在这个位置下棋
                ...
                score = self.evaluate_position(board, self.current_black_list, self.current_white_list, position_idx)
    ...
    def get_action(self, env, *args, **kwargs):
        if env.last_move == -1:
            m, n = (self.size // 2, self.size // 2)
            position_idx = m * self.size + n
            probs = np.zeros(self.size * self.size) / self.size * self.size
            probs[position_idx] = 1.
            return position_idx, probs
        else:
            result = self.select_move(env)
